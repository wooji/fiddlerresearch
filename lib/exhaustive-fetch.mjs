...
function proxyGroups(proxies) {
  return [
    proxies.filter(p => p.username?.startsWith('GRX')),      // Evomi mobile
    proxies.filter(p => p.username === 'WoojiWashed'),         // WoojiWashed ISP
    proxies.filter(p => p.username?.startsWith('xyz')),        // Byteful
...
export async function exhaustiveFetch(url, opts = {}) {
  ...
  // 1. CDP real browser (user's ATT connection)
  const cdp = await getCdpBrowser();
  if (cdp && !opts.skipCdp) {
    log('  [try] CDP real browser');
    try {
      const ctx = await cdp.newContext({ locale: 'en-US', viewport: { width: 1366, height: 900 } });
      const page = await ctx.newPage();
      const res = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
      if (res?.status() < 400) {
        const content = opts.expectJson
          ? await page.evaluate(() => JSON.parse(document.body.innerText))
          : await page.content();
        await page.close();
        log('  [OK] CDP');
        return content;
      ...
      await page.close();
    } catch (e) { log(`  [fail] CDP: ${e.message?.slice(0, 60)}`); }
  }

  // 2. CDP + proxy (each provider group)
  if (cdp && !opts.skipCdp) {
    for (const group of groups) {
      const proxy = randomFrom(group);
      log(`  [try] CDP+proxy ${proxy.host}`);
      try {
        const ctx = await cdp.newContext({
          locale: 'en-US', viewport: { width: 1366, height: 900 },
          proxy: { server: `http://${proxy.host}:${proxy.port}`, username: proxy.username, password: proxy.password },
        });
        const page = await ctx.newPage();
        const res = await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
        if (res?.status() < 400) {
          ...
          await page.close();
          log(`  [OK] CDP+proxy ${proxy.host}`);
          return content;
        }
        await page.close();
      } catch (e) { log(`  [fail] CDP+proxy ${proxy.host}: ${e.message?.slice(0, 60)}`); }
    }
  }

  // 3. Headed Playwright + each proxy group
  for (const group of groups) {
    const proxy = randomFrom(group);
...
export async function exhaustivePlaywright(url, fn, opts = {}) {
  ...
  // 2. Headed Playwright + each proxy group
  for (const group of groups) {
    const proxy = randomFrom(group);
...
export async function toppsShopifyProducts(collectionHandle, opts = {}) {
  ...
  const data = await exhaustiveFetch(url, { expectJson: true, log });
  if (!data) return null;
