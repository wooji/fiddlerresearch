...
export function legoSetScore(setKey) {
  ...
  const set  = sets[setKey];
  if (!set) return null;
  const prod = set.products?.['sealed'] ?? set.products?.[Object.keys(set.products ?? {})[0]];
  if (!prod) return null;

  const retail   = set.retail ?? LEGO_RETAIL_DEFAULT;
  const athMult  = prod.ath     ? prod.ath  / retail : 1;
  const curMult  = prod.current ? prod.current / retail : 1;
  const months   = prod.months ?? 1;
  const retired  = set.retired === true || set.eol != null;
