#!/usr/bin/env node
import { readFileSync, writeFileSync, appendFileSync } from 'fs';
import { chromium } from 'playwright-extra';
import StealthPlugin from 'puppeteer-extra-plugin-stealth';

chromium.use(StealthPlugin());

const DBs = {
  pokemon: 'set-history.json',
  one_piece: 'set-history-one-piece.json',
  lorcana: 'set-history-lorcana.json',
  sports: 'set-history-sports.json'
};

const LOG = 'tnt-fetch-cards.log';

function log(msg) {
  console.log(msg);
  appendFileSync(LOG, msg + '\n');
}

async function tntFetchCards(setName, category) {
  let browser;
  const cards = [];
  const date = new Date().toISOString().split('T')[0];

  try {
    browser = await chromium.launch({ headless: true });
    const page = await browser.newPage();
    page.setDefaultTimeout(10000);

    const query = `${setName} card`;
    const url = `https://www.trollandtoad.com/search/type:Magic/sort:popularity/${encodeURIComponent(query)}`;

    log(`    T&T: ${setName.slice(0, 50)}`);

    try {
      await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
    } catch (e) {
      log(`      [timeout]`);
    }

    // Extract products — Shopify-based, no Cloudflare
    const products = await page.evaluate(() => {
      const items = [];
      document.querySelectorAll('[data-product-id], .ProductCard, article').forEach(el => {
        const titleEl = el.querySelector('h2, a[href*="/products/"]');
        const priceEl = el.querySelector('[data-price], .price, [data-testid="price"]');

        const title = titleEl?.textContent?.trim();
        const priceStr = priceEl?.textContent?.trim();

        if (title && priceStr) {
          const match = priceStr.match(/\$?([\d,]+\.?\d*)/);
          const price = match ? parseFloat(match[1].replace(/,/g, '')) : null;

          if (price && price > 0 && price < 1000) { // Sanity: <$1000 per card
            items.push({
              title: title.slice(0, 100),
              price: parseFloat(price.toFixed(2))
            });
          }
        }
      });
      return items.slice(0, 30); // Top 30 products
    }).catch(e => {
      log(`      [eval] ${e.message.split('\n')[0]}`);
      return [];
    });

    if (products.length > 0) {
      products.forEach((prod, idx) => {
        cards.push({
          cardId: `${setName.replace(/\s+/g, '-')}-tnt-${idx}`,
          name: prod.title,
          market: prod.price,
          priceHistory: [
            { date, price: prod.price, source: 'trollandtoad' }
          ],
          fetchedAt: new Date().toISOString()
        });
      });
    }

    await browser.close();
  } catch (e) {
    log(`    [error] ${e.message.split('\n')[0]}`);
    if (browser) await browser.close().catch(() => {});
  }

  return cards;
}

async function main() {
  log(`[tnt-fetch-cards] ${new Date().toISOString()}`);

  let totalSets = 0, totalCards = 0;

  for (const [category, dbFile] of Object.entries(DBs)) {
    try {
      const db = JSON.parse(readFileSync(dbFile, 'utf8'));
      const sets = db.sets || db;
      const isNested = !!db.sets;

      log(`\n[${category}] ${Object.keys(sets).length} sets`);
      let catSets = 0, catCards = 0;

      for (const [setKey, set] of Object.entries(sets)) {
        const setName = set.label || set.name || set.set_name || setKey;

        const cards = await tntFetchCards(setName, category);

        if (cards.length > 0) {
          set.cards = set.cards || {};
          set.cards.fullCardList = [...(set.cards.fullCardList || []), ...cards];
          set.cards.fetchedAt = new Date().toISOString();

          catCards += cards.length;
          catSets++;
          totalCards += cards.length;

          const avgPrice = (cards.reduce((s, c) => s + c.market, 0) / cards.length).toFixed(2);
          log(`    ✓ ${setName}: ${cards.length} cards | avg $${avgPrice}`);

          await new Promise(r => setTimeout(r, 3000));
        }
      }

      const output = isNested ? db : sets;
      writeFileSync(dbFile, JSON.stringify(output, null, 2));

      log(`  [${category}] ${catSets} sets, ${catCards} cards`);
      totalSets += catSets;
    } catch (e) {
      log(`  ERROR ${category}: ${e.message}`);
    }
  }

  log(`\n[COMPLETE] ${totalSets} sets, ${totalCards} cards\n`);
}

main().catch(e => { log(`FATAL: ${e.message}`); process.exit(1); });
